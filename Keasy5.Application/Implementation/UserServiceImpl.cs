...
using Keasy5.Domain.Services;
using Keasy5.Domain.Specifications;
using Keasy5.ServiceContract;

namespace Keasy5.Application.Implementation
{
    /// <summary>
    ...
    public class UserServiceImpl : ApplicationService, IUserService
    {
        ...
        {
            return PerformUpdateObjects<UserDataObjectList, UserDataObject, User>(userDataObjects, 
                userRepository,
                udo => udo.ID,
                (u, udo) =>
                {
        ...
        public void DeleteUsers(UserDataObjectList userDataObjects)
        {
            ...
            foreach (var userDataObject in userDataObjects)
            {
                ...
                if (!IsEmptyGuidString(userDataObject.ID))
                    ...
                    throw new ArgumentNullException("userDataObject", "Either ID, UserName or Email should be specified.");
                var userRole = userRoleRepository.Find(Specification<UserRole>
                    .Eval(ur => ur.UserID == user.ID));
